function [Video_True_Rec] = makeRecG(video,cx,cy,hb,wb,W)
[hi, wi,~,nf]=size(video);
...
for i=1:nf
    ...
    Video_True_Rec(:,:,1,i)=squeeze(Video_True_Rec(:,:,1,i)).*uint8(~mask);
    Video_True_Rec(:,:,2,i)=squeeze(Video_True_Rec(:,:,2,i))+uint8(mask*inf);
